...
export class HexagonAdjectants {
	...
	constructor(x: number, y: number, readonly _ringLevel: number, readonly _id: number, orientation: BoardHexOrientation, r: number, padding: number) {
	...
	private initializeAdjectants(orientation: BoardHexOrientation) {
		for (let i = 0; i < 6; i++) {
			...
			const roundedHexAngle = AngleAndOrientationUtils.calculateRoundedHexAngle(adjX, adjY, this._hexData.x, this._hexData.y);
	...
	private getRingArrays(hexagonArray: Array<BoardHexDataInterface[]>): void {
	...
	pushAdjectant(type: 'adjectant' | 'bridge', currentAngle: number, hexagon: BoardHexDataInterface) {
		const targetArray = type === 'adjectant' ? this._adjectants[currentAngle].adjectantTo : this._adjectants[currentAngle].bridgeTo;
		targetArray.push({ ringLevel: hexagon.ringLevel, id: hexagon.id });
	}

	findCurrentPossibleHexagon(
	...
	findCorrespondingAdjectants(hexagonArray: Array<BoardHexDataInterface[]>) {
	...
	private findRegularAdjectants(hexagonArray: Array<BoardHexDataInterface[]>) {
		Object.keys(this._adjectants).forEach((angleKey) => {
			const currentAngle = parseInt(angleKey);
			const foundAdjectant = this.findCurrentPossibleHexagon(this._unifiedAdjectantRings, currentAngle, [0, 2], this._ringLevel === 5 ? 10 : 0);
	...
	private findSpecialAdjectants(hexagonArray: Array<BoardHexDataInterface[]>) {
		Object.keys(this._adjectants).forEach((angleKey) => {
			const currentAngle = parseInt(angleKey);
	...
	private findBridges(hexagonArray: Array<BoardHexDataInterface[]>, currentAngle: number) {
