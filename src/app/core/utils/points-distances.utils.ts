...
export class PointDistancesUtils {
	static getDistanceBetweenPoints(point1: { x: number; y: number }, point2: { x: number; y: number }): number {
		const dx = point2.x - point1.x;
		const dy = point2.y - point1.y;
		return Math.round(Math.sqrt(dx * dx + dy * dy));
	}

	static getMaxDistanceToHexagon(hexagon: BoardHexDataInterface, centerPoint: { r: number; padding: number }, factor: number): number {
		return centerPoint.r * factor * centerPoint.padding * hexagon.padding;
	}

	static isInBetweenHexagonDistances(
		...
		return (
			this.getDistanceBetweenPoints(centerPoint, hexagon) <= this.getMaxDistanceToHexagon(hexagon, centerPoint, upperFactor) &&
